...
// env FANCY_STORAGE=local|s3; s3 uses @aws-sdk/client-s3 to connect to any S3-compatible endpoint (FANCY_S3_*).
...
const DRIVER = process.env.FANCY_STORAGE ?? "local";
const LOCAL_DIR = process.env.FANCY_UPLOAD_DIR ?? path.join(os.homedir(), ".fancy-loop", "uploads");
...
// ── S3-compatible driver (disabled by default; enable by running `npm i @aws-sdk/client-s3` and setting FANCY_S3_ENDPOINT/BUCKET/REGION/KEY/SECRET) ──
async function s3client(): Promise<any> {
  ...
  return new mod.S3Client({
    endpoint: process.env.FANCY_S3_ENDPOINT, region: process.env.FANCY_S3_REGION ?? "us-east-1",
    forcePathStyle: true, // required for MinIO/Garage
    credentials: { accessKeyId: process.env.FANCY_S3_KEY ?? "", secretAccessKey: process.env.FANCY_S3_SECRET ?? "" },
  });
}
async function saveS3(key: string, stream: Readable): Promise<Saved> {
  ...
  await (await s3client()).send(new mod.PutObjectCommand({ Bucket: process.env.FANCY_S3_BUCKET, Key: key, Body: Buffer.concat(chunks) }));
  return { key, size };
}
async function readS3(key: string): Promise<Buffer> {
  const mod = await import("@aws-sdk/client-s3" as string);
  const r = await (await s3client()).send(new mod.GetObjectCommand({ Bucket: process.env.FANCY_S3_BUCKET, Key: key }));
